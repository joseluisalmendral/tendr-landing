 ...
 *
 * CTA is waitlist-honest: the product is pre-launch, so the header CTA points
 * to #waitlist (the only real conversion action) instead of a pricing anchor.
 *
 * anchorBase: optional base path for in-page section anchors (default "/").
 * A link like "#como-funciona" becomes `${anchorBase}#como-funciona`. When
 * anchorBase is "/" (default), next/link scrolls in-page on the home route and
 * navigates + anchors from any other page. Pass the current page's path when
 * that page owns the section ids (e.g. "/agencias" for the agencies vertical).
 * Non-anchor links (/pricing, /privacy, /terms) are never prefixed.
 *
...
const NAV_LINKS = [
  ...
  { label: "Funciones", anchor: "funciones" },
  { label: "Precios", anchor: "precios" },
] as const;

export function Header({ anchorBase = "/" }: { anchorBase?: string }) {
  // Normalise anchorBase: strip trailing slash so `${base}#id` never produces
  // "//#id" (for the default "/") — Next.js resolves "/#id" correctly.
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border-hairline bg-surface/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 w-full max-w-[1280px] items-center justify-between px-6 md:px-8">
        ...
        >
          <Link href={`${base}#waitlist`}>Únete a la waitlist</Link>
        </Button>
      </div>
    </header>
  );
